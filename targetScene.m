...
static NSString * const thisSceneName = @"targetScene";
static NSString * const nextSceneName = @"scores";
static NSString * const scoreName= @"_targetScore";
static NSString * const qName= @"_targetQ";
...
@implementation targetScene
{
    ...
    NSMutableArray *shapes;    int _targetQ;
...
- (void)spawnNewObstacle {
    ...
    for (CCNode *_balloon1 in _balloons) {
        ...
        remcount--;
        CCColor * shirtColor;
        double red2 = ((double)arc4random() / ARC4RANDOM_MAX);
        double green2 = ((double)arc4random() / ARC4RANDOM_MAX);
        double blue2 = ((double)arc4random() / ARC4RANDOM_MAX);
        Target* _balloon;
        _balloon=(Target*)_balloon1;
        ...
        if (choice == choiceColor) {
            ...
            shirtColor = [CCColor colorWithRed: cs.r/255.0 green:cs.g/255.0 blue:cs.b/255.0 ];
            [_balloon.target setColor: shirtColor];
        }else{
            
            shirtColor = [CCColor colorWithRed: red2 green:green2 blue:blue2];
        }
...
- (void)restart {
        CCScene *scene = [CCBReader loadAsScene:@"targetScene"];
...
- (void)balloontouch:(Target*) _balloon1
{
    if (BulletCount <=0){
        return;     }
    
    
    // for (Target *_balloon1 in _balloons) {
    ...
    hitRight.positionInPoints = _balloon1.positionInPoints;
     ...
     //   hitRight.position = ccp(_balloon1.balloon.position.x+100,_balloon1.balloon.position.y+100);
