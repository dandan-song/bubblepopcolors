...
static const CGFloat distanceBetweenObstacles = 80.f;
static const CGFloat firstObstaclePosition =0.0f;
static const CGFloat firstObstacleYPosition = 150.0f;
static NSString * const thisSceneName = @"airPlanes";
static NSString * const nextSceneName = @"targetScene";
static NSString * const scoreName= @"_airPlaneScore";
static NSString * const qName= @"_airPlaneQ";
...
- (void)spawnNewObstacle:(CGFloat)extray {
    ...
    CGSize s = [CCDirector sharedDirector].viewSize;
    CGFloat offs = s.width;

    CCNode*previousObstacle = [_balloons lastObject];
    CGFloat previousObstacleXPosition = previousObstacle.position.x;
    CGFloat previousObstacleYPosition = previousObstacle.position.y;
    if ((!previousObstacle)||(previousObstacleYPosition>(s.height*3/4))) {
        // this is the first obstacle
        previousObstacleXPosition = firstObstaclePosition;
        previousObstacleYPosition = firstObstacleYPosition;
    }
    
    CCNode *obstacle = [CCBReader load:@"airPlane"];
    obstacle.zOrder = DrawingOrderBalloons;
    
     
    obstacle.position = ccp((1.2*offs)+obstacle.contentSize.width/2.0+(r-1)*distanceBetweenObstacles,
                            previousObstacleYPosition+extray);
    ...
    [obstacle setCascadeColorEnabled:true];
    [obstacle setColor: shirtColor];
...
- (void)didLoadFromCCB {
   ...
   //    _scope.physicsBody.sensor = TRUE;
   ...
   // NSNumber* myRandomTagName1 = [tagNameArray1 objectAtIndex:r1];
    ...
    match_tag1= [NSNumber numberWithInt:r1];
    ...
    for (int i=0;i<count;i++){
        [self spawnNewObstacle:s.height/2];
    }
...
-(void)removeBalloons;
{
    ...
    for (airPlane *obstacle in _balloons) {
...
- (void)balloontouch:(airPlane*) _balloon1
{
    if (BulletCount <=0){
        return;     }
